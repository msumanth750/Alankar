...
def sale(datea, dateb):
    ...
    ttl = int(0 if exps.aggregate(Sum('amount'))[
              'amount__sum'] is None else exps.aggregate(Sum('amount'))['amount__sum'])
    sales = []
    salet = 0
    cont = {}
    for brand in brands:
        ...
        salet += cont['amt']
        sales.append(cont)
    context = {'sales': sales, 'salet': salet, 'date': datea,
               'exps': exps, 'ttl': ttl, 'dateb': dateb, 'form': SaleForm()}
    return context
...
class PdfView(View):
    def get(self, request, *args, **kwargs):
        ...
        pdf = render_to_pdf('outsale/pdf.html', sale(datea,
                            datea + datetime.timedelta(days=1)))
